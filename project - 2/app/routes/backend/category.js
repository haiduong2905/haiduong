...
const CategoryModel = require(__path_models + collection);
const UsersModel = require(__path_models + 'users');
const UtilsHelpers = require(__path_helpers + 'utils');
const ParamsHelpers = require(__path_helpers + 'params');
const ValidateCategory = require(__path_validates + collection);
...
router.get('(/status/:status)?', async(req, res, next) => {
    ...
    await CategoryModel.countItems(params).then((data) => { // Đếm số phần tử có trong dữ liệu trả về 
...
router.get('/delete/:id', (req, res, next) => {
    ...
    CategoryModel.deleteItems(id, { task: 'delete-one' }).then((result) => {
...
router.post('/delete', (req, res, next) => {
    let id = ParamsHelpers.getParam(req.params, 'id', '');
    // Đổi status active <-> inactive
    CategoryModel.deleteItems(req.body.cid, { task: 'delete-multi' }).then((result) => {
        req.flash('success', util.format(Notify.DELETE_MULTI_SUCCESS, result.n), false);
        res.redirect(linkIndex);
    });
});

//GET FORM Add, Edit Category
router.get('/form(/:id)?', (req, res, next) => {
    ...
    if (id === '') { // Thêm mới
        ...
        CategoryModel.getItems(id, null).then((item) => {
...
router.post('/save', (req, res, next) => {
    ...
    if (errors) {
        ...
        CategoryModel.saveItems(item, { task: taskCurrent }).then((result) => {
            if (taskCurrent == 'add') {
